...
echo -e "$White***********************************************"
echo "         Compiling Synthetic Kernel             "
echo -e "***********************************************$nocol"

LC_ALL=C date +%Y-%m-%d
kernel_dir=$PWD
build=$kernel_dir/out
export CROSS_COMPILE="/home/rostifaner/android/toolchain/aarch64-linux-android-4.9/bin/aarch64-linux-android-"
kernel="Synthetic"
version="1.0"
vendor="xiaomi"
android="OREO"
...
zip=zip
date=`date +"%Y%m%d-%H%M"`
config=tissot_defconfig
kerneltype="Image.gz-dtb"
jobcount="-j$(grep -c ^processor /proc/cpuinfo)"
#modules_dir=$kernel_dir/"$zip"/system/vendor/lib/modules
modules_dir=$kernel_dir/"$zip"/modules
zip_name="$kernel"-"$version"-"$date"-"$android"-"$device".zip
export KBUILD_BUILD_USER=rostifaner
export KBUILD_BUILD_HOST=Qbic
